...
%% 8. 终极三合一可视化（全口径绝对相位域版本）
% 修复内容：
% 1) 横坐标统一使用 radius_stop，确保显示的是 HIK 大口径全口径范围
% 2) 实测平均曲线改为“绝对相位 = Ideal + ResidualMean”，不再直接画残差
% 3) 拟合曲线也统一画在绝对相位域：Ideal + ResidualFit
% 4) 给出更合理的 RMSE / R^2 统计

fprintf('\n>>> Step 8: 生成全口径对比图（统一绝对相位域）...\n');

% ================= 参数控制 =================
Num_Bins           = 500;    % 径向平均分箱数
USE_OUTLIER_REJECT = false;  % Zemax 理想验证建议 false
Outlier_Thresh     = 2.5;    % 仅当 USE_OUTLIER_REJECT=true 时有效
USE_SMOOTH         = false;  % Zemax 理想验证建议 false
Scatter_Step       = 10;     % 散点稀疏显示步长
% ===========================================

% -------------------------------------------------------------------------
% 1. 构造理想相位曲线（绝对相位）
% -------------------------------------------------------------------------
r_half = linspace(0, radius_stop * 1e3, 1000);   % mm
r_norm_half = r_half ./ 1.0;                     % Zemax 归一化半径 = 1 mm
...
% -------------------------------------------------------------------------
% 2. 构造拟合残差曲线，并转成绝对相位曲线
%    Step 7 得到的 A_Final_Zemax 对应的是“残差拟合系数”
% -------------------------------------------------------------------------
...
Phi_FitAbs_Half = Phi_Ideal_Half + Phi_ResFit_Half;   % 绝对拟合曲线

% -------------------------------------------------------------------------
% 3. 构造散点数据
%    X_dat, Y_dat, Z_dat 来自 Step 7：
%    其中 Z_dat 是“相对理想相位”的残差数据（Phase_Target）
% -------------------------------------------------------------------------
R_scatter_raw = sqrt((X_dat - best_shift(1)).^2 + (Y_dat - best_shift(2)).^2); % mm
Z_scatter_raw = Z_dat - Coeffs_Stable(1);   % 去除 piston，只保留残差主体

% 可选：离群点剔除（一般 Zemax 理想数据建议关闭）
if USE_OUTLIER_REJECT
    Z_ref_vals = interp1(r_half, Phi_ResFit_Half, R_scatter_raw, 'linear', 'extrap');
    resid_abs  = abs(Z_scatter_raw - Z_ref_vals);
    valid_mask_scatter = resid_abs < Outlier_Thresh;
...
% -------------------------------------------------------------------------
% 4. 径向平均，得到 residual mean
% -------------------------------------------------------------------------
edges = linspace(0, max(r_half), Num_Bins + 1);
z_mean_res = zeros(Num_Bins, 1);   % 残差均值
...
% 5. 计算统计量
% -------------------------------------------------------------------------
% (1) 残差域 R^2：比较 residual mean 与 residual fit
z_fit_interp = interp1(r_half, Phi_ResFit_Half, r_centers, 'linear', 'extrap')';

SSR = sum((z_mean_res - z_fit_interp).^2);
SST = sum((z_mean_res - mean(z_mean_res)).^2);
if SST > 0
    R_Square = 1 - SSR / SST;
else
    R_Square = 1;
...
% (2) 相对理想绝对相位的加权 RMSE
err_phi = Phi_MeasAbs_Centers - Phi_Ideal_On_Centers(:);   % 实际上就是 z_mean_res
valid_w = bin_count > 0;
if any(valid_w)
    RMSE_phi = sqrt(sum(bin_count(valid_w) .* (err_phi(valid_w).^2)) / sum(bin_count(valid_w)));
else
    RMSE_phi = NaN;
end

% (3) 2D 像素级 RMSE（与 Figure 1 的口径定义一致）
IdealPhase2D = exp(1i * phi_init);
phase_err_2d = angle(U_final .* conj(IdealPhase2D));
Mask2D = (Rho_sim <= 0.98 * radius_stop);
...
z_scat_full = [ Z_scatter; Z_scatter];   % 这里仍然是 residual scatter
...
xlim([-radius_stop*1e3, radius_stop*1e3]);   % 关键：全口径横坐标
...
str_stats = {
    ...
    ['']
    ['\bf Deviation from Ideal (Phase):']
    [' $RMSE_{\mathrm{1D}} = ' num2str(RMSE_phi, '%.4f') ' \ \mathrm{rad}$']
    [' $RMSE_{\mathrm{2D}} = ' num2str(RMSE_phi_2D, '%.4f') ' \ \mathrm{rad}$']
};
annotation('textbox', [0.15 0.15 0.26 0.18], 'String', str_stats, ...
...
fprintf('Step 8 完成：\n');
fprintf('  - 横坐标范围 = [%.3f, %.3f] mm\n', -radius_stop*1e3, radius_stop*1e3);
fprintf('  - 1D RMSE = %.6f rad\n', RMSE_phi);
fprintf('  - 2D RMSE = %.6f rad\n', RMSE_phi_2D);
fprintf('  - R^2 = %.6f\n', R_Square);
