...
%% 7. 终极优化：严格圆对称拟合（残差域自洽版）
% 核心思想：
% 1) Step 5 的 U_final 已经是去除了 piston/tilt/defocus 后的结果
% 2) 此处直接构造“相对理想相位残差”：
%       Phase_Target = angle(U_final .* conj(exp(1i*phi_init)))
% 3) Step 8 再用 Ideal + Residual 的方式恢复到绝对相位域绘图
% 这样和 Figure 1 的误差定义一致，避免 fig2 中把 ideal 重复加一遍

fprintf('\n>>> Step 7: 启动严格圆对称优化（残差域自洽版）...\n');

% -------------------------------------------------------------------------
% 1. 构造残差相位目标：Δphi(x,y) = measured - ideal
% -------------------------------------------------------------------------
U_ideal_phase = exp(1i * phi_init);
Phase_Target  = angle(U_final .* conj(U_ideal_phase));   % wrapped residual in [-pi, pi]

% 有效掩膜：口径内 + 振幅足够高
Amp = abs(U_final);
AmpMask  = Amp > 0.20 * max(Amp(:));
Fit_Mask = (Rho_sim <= 0.98 * radius_stop) & AmpMask;
...
% -------------------------------------------------------------------------
% 2. 自动搜索最佳光轴中心
%    这里固定拟合半径为真实物理半径 radius_stop
% -------------------------------------------------------------------------
R_fixed_mm = radius_stop * 1e3;
start_params = [0, 0];
...
% -------------------------------------------------------------------------
% 3. 获取最终拟合系数
%    Coeffs_Stable(1) = piston residual
...
%% 8. 全口径三合一可视化（残差域自洽版：Ideal + Residual）
fprintf('\n>>> Step 8: 生成全口径对比图（残差域自洽版）...\n');
...
% -------------------------------------------------------------------------
% 3. 散点数据：这里 Z_dat 就是残差 Δphi(x,y)
% -------------------------------------------------------------------------
R_scatter_raw = sqrt((X_dat - best_shift(1)).^2 + (Y_dat - best_shift(2)).^2); % mm
Z_scatter_raw = Z_dat - Piston_Final;   % 去掉残差常数项
